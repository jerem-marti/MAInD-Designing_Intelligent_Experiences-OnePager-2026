...
- The Invitation section's CTA must link to the team's **live testable agent**. The agent URL should be a clearly documented environment variable or configuration value (e.g. `WEMATCH_AGENT_URL` in `.env`) with a placeholder set during the build. The team will replace the placeholder with the real URL once the agent is hosted.
- The CTA copy uses a *try* verb — "Meet WeMatch," "Talk to WeMatch," or "Try WeMatch" — not a signup verb. Refer to the script for the verb shift rationale.
- The Invitation section must include the expectation-setting micro-line below the CTA (proposed: *"A live preview. Real agent. No signup."* — final wording per the script).
